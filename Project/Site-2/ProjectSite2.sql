...
create or replace package body mypackProcedure AS
    ...
    BEGIN
		...
		update RecordList@site1 set teamName=teamNameUpdate,country=countryUpdate,seasonID=seasonIDUpdate,
	...
	if UPPER(operationType)='SEARCH' then
	    ...
	    if UPPER(countrywise)='BOTH' then
		  if UPPER(seasonwise)='ALL' then
		    ...
		    End if;
		End  if;
		insert into RecordList@site1 values(recID,teamName,countryname,
